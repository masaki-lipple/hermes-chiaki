...
- 戸田さんが `@Chiaki AI 〜`（どこでも・何でも）／#8902・#5902 の投稿 → `chiaki-tuning` が Haiku で振り分け、**必ず一度きいてから**起票（2ターン）。会話エージェントは使わない。
  ...
  - **rule**（言葉のルール＝旧 soft：トーン/用語/表記）→ Notion **`Rule Registry_Hermes Agent__DB`**（database_id `e10777d5a7a04ac294273b9e077e1a38`・未承認・種別 用語/レギュレーション/スタイル・起票者/対象=小文字 chiaki）。承認→正本（用語辞書/レギュレーション/Style）→翌日 sync で反映。
