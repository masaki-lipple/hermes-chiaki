...
ACTIONS = {
    "propose": "新しい指摘・依頼を起票の確認にかける。proposals に案（type=issue|rule・issue_kind=バグ|変更|新機能|その他・"
               ...
               "reply で案の中身を自然に示し、「Issueとして処理しますか？」と確認する（2026-07-21 戸田指定の聞き方。"
               "表記は英字の「Issue」＝カタカナで「イシュー」と書かない）。"
    ...
    "file": "提示済みの案の登録を承認された（OK/はい/Issueに追加で 等。他の話が同居していても承認が含まれていれば file）。"
            ...
            'issueの場合 "codex": true/false。true は戸田さんが実装の開始まで明示したときだけ'
            "（「Codexに回して」「実装して」等）。「イシューに入れておこう」「起票だけ」「いったん」"
            "「あとで」は false＝起票のみが既定（2026-08-07 戸田「イシューに追加するだけでいいのに"
            "Codexまわしはじめた、これ危険すぎる」。最終判定はシステムが実文で行う）。"
            "codex=true のときは「Codexに回す・進捗はこのスレッドに報告する」旨も reply の中で自然に伝える"
            "（システムは文を足さない＝あなたの一文で完結させる）。"
