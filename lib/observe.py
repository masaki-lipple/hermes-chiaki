...
def _protect_noncontent(text: str, transform):
    ...
    def _stash(mm):
        ...
        return "\x00"
    out = transform(_NONCONTENT.sub(_stash, text))
    for t in toks:
    ...
    def _apply(s):
        for f, sg in subs:
            if f in s:
                ...
                applied.append((f, sg))
        return s
    fixed = _protect_noncontent(text, _apply)  # URL/リンク/コード内は壊さない
...
def apply_regulations(text: str, reg: dict, scene: str = "社内コミュニケーション",
    ...
    def _do_subs(s):  # URL/リンク/コードは退避済み（_protect_noncontent）＝URL内を壊さない
        for f in findings:
        ...
        return s
    fixed = _protect_noncontent(text, _do_subs)
    ...
    _JP = r"[぀-ヿ㐀-鿿々〆]"
    # URL/メンション<...>/コード`...`は alnum を含むので、境界判定の前にマスクして接着・破壊を防ぐ。
    _toks = []

    def _stash(mm):
        _toks.append(mm.group(0))
        return f"\x00{len(_toks) - 1}\x00"
    masked = re.sub(r"<[^>\n]+>|https?://\S+|`[^`\n]+`", _stash, fixed)
    nb = re.sub(rf"([A-Za-z0-9]) +({_JP})", r"\1\2", masked)
    nb = re.sub(rf"({_JP}) +([A-Za-z0-9])", r"\1\2", nb)
    for _i, _t in enumerate(_toks):
        nb = nb.replace(f"\x00{_i}\x00", _t)
    if nb != fixed:
        fixed = nb
