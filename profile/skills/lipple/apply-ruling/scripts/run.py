...
# 理由付き却下（「却下。これは本当の人物名。」）＝先頭が却下語＋続きの理由文。却下として裁定し、
# 理由は reject_learn.jsonl へ学習＝同じ指摘を繰り返さない（2026-07-23 戸田「次回指摘しないように
# する仕組みをつくる」。従来は interpret 扱いで裁定されず item が pending のまま残っていた=7/20実例）
_REJECT_LEAD_RE = re.compile(r"^(?:却下|見送り|ボツ|ぼつ|NG|ng|やめ)[。、．.！!\s　]+(\S[\s\S]*)$")
# お礼・相づち（裁定でも指示でもない）＝skip。「ありがとう！」だけの返信が interpret 扱いで
# Haiku文面生成→対象スレッドへの投稿まで進み得た穴の封鎖（2026-07-23 監査レビューで確定・
# #8902で指摘事項として報告済みの件）
ACK_EXACT = {"ありがとう", "ありがとうございます", "ありがとうございました", "ありがと", "あざす",
             "感謝", "感謝です", "助かる", "助かります", "助かりました", "たすかる", "たすかります",
             "さすが", "さすがです", "ナイス", "おつかれさま", "おつかれさまです", "おつかれさまでした",
...
def _classify(text: str):
    ...
    if (core in REJECT_EXACT or core.endswith(_REJECT_SUFFIX)) and not _EDIT_VERB_RE.search(t):
        return ("reject", "")
    if core in ACK_EXACT:
        return ("skip", "")  # お礼・相づち＝指示ではない（interpret誤実行の防止）
    lead = _REJECT_LEAD_RE.match((text or "").strip())
    if lead and not _EDIT_VERB_RE.search(lead.group(1)):
        return ("reject", lead.group(1).strip())  # 理由付き却下＝payloadに理由（学習に回す）
...
def _digest(s: str) -> str:
    """承認バインディング用の内容ダイジェスト（R3・aikoの承認証明の型）。"""
...
def _rule_one(pend: dict, tts: str, it: dict) -> int:
    ...
    if not toda:
        return 0
    # 裁定発話の選定（2026-07-21 監査⑥b→2026-07-23 レビューで再修正）: 新しい順に見て、
    # お礼・質問・相づち（skip）は飛ばし、最初の「実質的な発話」を採用する。それが裸のGO/却下なら
    # 裁定として実行、編集指示（interpret）ならそれを優先する。
    ...
    if verdict == "reject":
        ...
        if reason:
            # 理由付き却下＝検知そのものが誤り（人名等）の明示＝学習して同じ指摘を繰り返さない
            # （propose/typo-scanが参照）。裸の「却下」は「今回は不要」の意味もある＝学習しない
            try:
                runtime.append_jsonl("reject_learn.jsonl", {
                    "ts": runtime.now_ts(), "kind": it.get("finding_kind", ""),
                    "found": it.get("verify_found") or "", "reason": reason,
                    "draft": (it.get("draft") or "")[:200], "thread": tts})
            except Exception as e:
                print(f"[apply-ruling] reject_learn書き込み失敗（続行）: {e}")
            report = "了解です。今回は出しません。理由も学習して、同じ指摘を繰り返さないようにします。"
        ...
        if report:
            # 承認バインディング（R3）: どの提案(tts)のどの内容(digest)への裁定かを台帳に束縛
            binding = {"proposal": tts, "digest": _digest(draft), "approver": runtime.TODA,
        ...
        tries = int(led.get("tries") or 0) + 1
        if tries > 6:
            # 投稿の恒久失敗（アーカイブ・権限等）＝無限リトライにせず1回だけ知らせて終端
            # （2026-07-23 監査レビュー: クレーム失効→再投稿の試行が10分毎に無期限継続していた）
            it["status"], it["ruling_text"] = "unactionable", ruling_text
            binding = {"proposal": tts, "digest": _digest(draft), "approver": runtime.TODA,
                       "ruled_at": runtime.now_ts(), "verdict": "unactionable"}
            it["approval"] = binding
            _save(pend)
            ledger.record(eid, actor=runtime.TODA, ch=runtime.CH_CHIAKI_MGMT, thread_root=tts,
                          ts=ruling_ts, kind="ruling", owner="apply", status="ruled",
                          refs={"item_status": "unactionable", "approval": binding})
            runtime.append_jsonl("rulings.jsonl", {
                "ts": runtime.now_ts(), "thread_ts": tts, "verdict": "unactionable",
                "kind": it.get("finding_kind", ""), "original": draft,
                "final_text": "", "ruling_text": ruling_text})
            source.post_thread_reply(runtime.CH_CHIAKI_MGMT, tts,
                                     f"<@{runtime.CHIAKI_SELF}>\n対象スレッドへの投稿が繰り返し失敗する"
                                     "ため、この提案はいったんクローズします。チャンネルの状態"
                                     "（アーカイブ・権限）の確認が必要かもしれません。")
            return 1
        ledger.record(eid, actor=runtime.TODA, ch=runtime.CH_CHIAKI_MGMT, thread_root=tts,
                      ts=ruling_ts, kind="ruling", owner="apply", status="ruling", tries=tries,
    ...
    # 承認バインディング（R3）: 実際に対象スレッドへ出した最終文面のdigestに束縛（GO=draft・
    # 文面修正=interpret後の文）。「何への承認だったか」が台帳と裁定台帳の両方に残る
    binding = {"proposal": tts, "digest": _digest(final or draft), "approver": runtime.TODA,
...
def _ledger_sweep(pend: dict) -> None:
    """apply所有の台帳行の定期掃除（2026-07-23 監査レビュー＝③の残穴）。終端瞬間の一括掃除だけでは
    ①提案スレッド（#8902）側の非裁定返信（GO後のお礼等） ②完了後に対象スレッドへ届く返信
    ③reject/gone等で閉じた提案の行 が誰にも終端されず恒久receivedで残り、Notion控えに「受信」が
    蓄積していた。itemの状態から「もう処理されない」行を判定してskippedで終端する。
    失効した消費開始クレーム（ruling）の残置も掃除（正常系はruledが上書き済み＝1時間超は異常残置）。"""
    try:
        items = pend.get("items", {})
        by_src = {(v.get("source_channel"), v.get("source_ts")): v for v in items.values()
                  if v.get("source_channel") and v.get("source_ts")}
        term = ("completed", "reject", "stale", "gone", "retracted", "already_fixed", "unactionable",
                "expired")
        now = runtime.now_ts()
        for eid, r in ledger.load().items():
            if r.get("owner") != "apply":
                continue
            st, age = r.get("status") or "received", now - float(r.get("at") or 0)
            if st == "ruling" and age > 3600:
                ledger.record(eid, status="skipped", note="失効クレームの掃除")
                continue
            if st != "received":
                continue
            ch, root = r.get("ch"), r.get("thread_root")
            it = items.get(root) if ch == runtime.CH_CHIAKI_MGMT else by_src.get((ch, root))
            if it is None:
                if age > 86400:  # 対応するitemが無い（古い提案等）＝1日たったら終端
                    ledger.record(eid, status="skipped", note="対応item無し（掃除）")
                continue
            if (ch == runtime.CH_CHIAKI_MGMT and it.get("status") != "pending") or it.get("status") in term:
                ledger.record(eid, status="skipped", note="裁定済み/終端itemのスレッド掃除")
    except Exception as e:
        print(f"[apply-ruling] sweep失敗（続行）: {e}")


_EXPIRE_PENDING_SEC = 2 * 86400  # 裁定が無い提案の自動失効（2026-07-23 戸田「2日で自動失効」＝
#   投稿せず閉じる。承認扱いの自動執行ではない＝「未承認は本番不反映」の原則は維持）


def _phase_expiry(pend: dict) -> int:
    """Phase3: 2日間裁定が無い pending 提案の自動失効（クローズ・スレッドへ1回だけ通知）。
    2026-07-23 戸田決定＝裁定待ちを無期限に積まない（実測17件・最古29日が滞留していた）。"""
    acted = 0
    now = runtime.now_ts()
    for tts, it in pend.get("items", {}).items():
        if it.get("status") != "pending":
            continue
        try:
            if now - float(tts) < _EXPIRE_PENDING_SEC:
                continue
        ...
        acted += 1
        runtime.append_jsonl("rulings.jsonl", {
            "ts": now, "thread_ts": tts, "verdict": "expired",
            "kind": it.get("finding_kind", ""), "original": it.get("draft", ""),
            "final_text": "", "ruling_text": ""})
        try:
            source.post_thread_reply(runtime.CH_CHIAKI_MGMT, tts,
                                     f"<@{runtime.CHIAKI_SELF}>\n2日間裁定が無かったため、この提案は"
                                     "自動で失効にしました（対象スレッドへの投稿はしていません）。"
                                     "必要でしたら、もう一度お知らせください！")
        except Exception as e:
            print(f"[apply-ruling] expiry notice failed {tts}: {e}")
    return acted
...
def main():
    ...
    expired = _phase_expiry(pend)  # 裁定フェーズの後＝ぎりぎりの裁定が失効に負けない
    if ruled or completed or expired:
        runtime.save_json("pending_approvals.json", pend)
    print(f"[apply-ruling] ruled={ruled} completed={completed} expired={expired}")
