---
name: ledger-notion
description: 実行台帳（exec_ledger.jsonl）をExecution_Chiaki_AI_DB（Notion）へ日次同期。1依頼=1行・IDで重複防止・状態が変わった行は更新。決定論・LLM非起動。
metadata:
  hermes:
...
本スキルは閲覧用の控えとして、NotionのExecution_Chiaki_AI_DBへ日次で写す（aiko §13「日中は同期書き込みしない・
